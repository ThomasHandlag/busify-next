  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      authorize: async (credentials) => {
        ...
        if (tokens && tokens.result) {
          return {
            id: tokens.result.id,
            fullName:  tokens.result.fullName, // Sử dụng fullName nếu API trả về
            email: tokens.result.email,
            phoneNumber: tokens.result.phoneNumber || "", // Thêm phoneNumber
            accessToken: tokens.result.accessToken,
            refreshToken: tokens.result.refreshToken,
          } as UserType;
        }
    ...
    async signIn({ user, account }) {
      if (account?.provider === "google") {
        try {
          ...
          user.accessToken = data.result.accessToken;
          user.refreshToken = data.result.refreshToken;
          user.phoneNumber = data.result.phoneNumber || ""; // Thêm phoneNumber
          user.fullName = data.result.fullName|| user.fullName; // Sử dụng fullName nếu có
