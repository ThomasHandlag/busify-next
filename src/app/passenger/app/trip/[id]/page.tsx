  ...
  for (let floor = 1; floor <= busLayout.floors; floor++) {
    for (let row = 1; row <= busLayout.rows; row++) {
      for (let col = 0; col < busLayout.cols; col++) {
        ...
        // Fallback to "available" if no tripSeatsStatus or no matching seat
        const seatStatus =
          tripSeatsStatus?.seatsStatus.find((s) => s.seatNumber === seatName)
            ?.status || "available"; // Thay "booked" bằng "available"
...
export default async function TripDetailPage({
  ...
  let tripDetail;
  try {
    tripDetail = await getTripDetail(Number(tripId));
    console.log("Fetched trip detail:", tripDetail);
  } catch (error) {
    console.error("Failed to fetch trip detail:", error);
    return <div>Đã xảy ra lỗi khi tải thông tin chuyến xe. Vui lòng thử lại sau.</div>;
  }

  if (!tripDetail || !tripDetail.bus) {
    console.log("Invalid trip detail data:", tripDetail);
    return (
      <div>
        Không tìm thấy thông tin chuyến xe hoặc dữ liệu không hợp lệ.
        <br />
        Trip ID: {tripId} | Dữ liệu nhận được: {JSON.stringify(tripDetail)}
      </div>
    );
  }
  ...
  let tripSeatsData;
  try {
    tripSeatsData = await getTripSeatById(Number(tripId));
    console.log("Fetched trip seats data:", tripSeatsData);
  } catch (error) {
    console.error("Error fetching trip seats:", error);
    tripSeatsData = null; // Fallback nếu lỗi
  }

  let busLayout;
  try {
    busLayout = await getBusSeatsLayout(tripDetail.bus.id);
    console.log("Retrieved bus layout:", busLayout);
  } catch (error) {
    console.error("Error fetching bus layout:", error);
    return <div>Không thể tải bố trí ghế ngồi. Vui lòng thử lại sau.</div>;
  }

  if (!busLayout) {
    return <div>Không tìm thấy bố trí ghế ngồi hợp lệ.</div>;
  }
  ...
  console.log("Generated bus seats:", busSeats);

  const bookingBar = (
    <MobileBookingBar
    tripId={tripId} // Truyền tripId
      layout={busLayout}
      seats={busSeats}
